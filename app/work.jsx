...
const PREVIEW_TRACKS = [
  {
    ...
    slots: [
      { id: 'barbershop', title: 'Barbershop Brand & Booking', stat: '2× Online Bookings', icon: '✂', src: 'more%20previews/barbershop.mov' },
      { id: 'childcare', title: 'Childcare Center', stat: '3× Inquiry Volume', icon: '☀', src: 'more%20previews/childcare.mov' },
      { id: 'rental', title: 'Short-Term Rental', stat: '+85% Direct Bookings', icon: '⌂', src: 'more%20previews/rental.mov' },
      { id: 'forge-quest', title: 'Bonus Game Preview', stat: 'Featured Game Demo', icon: '▣', game: true },
    ],
  },
  {
    id: 'automations',
    ...
    slots: [
      { id: 'followup', title: 'AI Follow-Up for Realtors', stat: 'Reply time: 9.2 hrs → 47 s', icon: '⚡', src: 'assets/reels/followup.mp4' },
      { id: 'medspa', title: 'Med Spa Booking Funnel', stat: '48% Booking Conversion', icon: '✚', src: 'assets/reels/medspa.mp4' },
      { id: 'dispatch', title: 'Home Services Dispatch', stat: '14 hrs/week Saved', icon: '🔧', src: 'assets/reels/dispatch.mp4' },
      { id: 'dashboard', title: 'Client Ops Dashboard', stat: '30 Agents Onboarded', icon: '▦', src: 'assets/reels/dashboard.mp4' },
    ],
  },
];
...
const FQ = {
  W: 320, H: 180, GROUND: 150,
  GRAVITY: 900, JUMP: -330,
  BASE_SPEED: 120, SPEED_PER_LVL: 14,
  SECTIONS: 5, SCORE_PER_SECTION: 120,
  COLORS: {
    skyTop: '#0B141E', skyBot: '#10202E', ridge: '#0E1B27', ridge2: '#13232F',
    ground: '#1A2B38', groundEdge: '#E3B65C', dash: 'rgba(225,224,204,0.25)',
    ink: '#E1E0CC', gold: '#E3B65C', bug: '#C4523F', bugDark: '#8E3527',
    site: 'rgba(227,182,92,0.16)', siteOn: 'rgba(227,182,92,0.55)',
    heart: '#C4523F', skin: '#D9A066', body: '#2E4356',
  },
};
...
function ForgeGame() {
  ...
  const reset = () => {
    g.current = {
      t: 0, last: 0, dist: 0, coins: 0, lvl: 1, hearts: 3,
      y: FQ.GROUND, vy: 0, grounded: true, inv: 0, runFrame: 0,
      bugs: [], leads: [], spawnBug: 1.4, spawnLead: 0.9,
      shipped: 0, flash: 0,
    };
  };
  ...
  const jump = () => {
    ...
    if (s.grounded) { s.vy = FQ.JUMP; s.grounded = false; }
  };
  ...
  useEffectW(() => {
    ...
    const drawBackdrop = (s) => {
      ...
      ctx.fillStyle = grad; ctx.fillRect(0, 0, FQ.W, FQ.H);
      ...
      for (let x = -80; x < FQ.W + 80; x += 80) {
        const bx = x - off1;
        ctx.fillRect(bx, 96, 34, 54); ctx.fillRect(bx + 42, 110, 26, 40);
      }
      const off2 = s ? (s.dist * 0.35) % 100 : 0;
      ctx.fillStyle = C.ridge2;
      for (let x = -100; x < FQ.W + 100; x += 100) {
        const bx = x - off2;
        ctx.fillRect(bx, 118, 40, 32); ctx.fillRect(bx + 52, 128, 30, 22);
      }
    };
    ...
    const drawSite = (s) => {
      ...
      const bx = 226, by = 48, bw = 78, bh = 62;
      px(bx - 3, by - 10, bw + 6, bh + 13, 'rgba(14,24,34,0.85)');
      ctx.strokeStyle = 'rgba(227,182,92,0.4)'; ctx.lineWidth = 1;
      ctx.strokeRect(bx - 2.5, by - 9.5, bw + 5, bh + 12);
      px(bx - 3, by - 10, bw + 6, 7, 'rgba(227,182,92,0.25)');
      px(bx, by - 8, 3, 3, C.bug); px(bx + 5, by - 8, 3, 3, C.gold); px(bx + 10, by - 8, 3, 3, C.ink);
      // Progress through the CURRENT build; resets after each ship (new client).
      const built = s
        ? Math.min(FQ.SECTIONS, Math.floor((score(s) % FQ.SCORE_PER_SECTION) / (FQ.SCORE_PER_SECTION / FQ.SECTIONS)))
        : 0;
      const rects = [
        [bx, by, bw, 8],            // header
        [bx, by + 11, bw, 20],      // hero
        [bx, by + 34, 37, 16],      // card L
        [bx + 41, by + 34, 37, 16], // card R
    ...
    const drawSmith = (s) => {
      ...
      if (legA) { px(x + 1, y + 16, 4, 2, C.body); px(x + 8, y + 14, 3, 4, C.body); }
      // torso + apron
      ...
      if (s.grounded) { px(x + 11, y + 4, 6, 2, C.skin); px(x + 16, y + 1, 3, 7, C.gold); }
      else { px(x + 11, y - 2, 2, 6, C.skin); px(x + 9, y - 8, 7, 4, C.gold); }
    };
    ...
    const step = (dt) => {
      ...
      s.vy += FQ.GRAVITY * dt;
      s.y += s.vy * dt;
      if (s.y >= FQ.GROUND) { s.y = FQ.GROUND; s.vy = 0; s.grounded = true; }

      // spawns
      s.spawnBug -= dt;
      if (s.spawnBug <= 0) {
        s.bugs.push({ x: FQ.W + 12, t: 0 });
        s.spawnBug = 1.1 + Math.random() * 1.2 - Math.min(0.5, s.lvl * 0.06);
      }
      s.spawnLead -= dt;
      if (s.spawnLead <= 0) {
        s.leads.push({ x: FQ.W + 12, y: FQ.GROUND - 20 - Math.random() * 45, t: 0 });
        s.spawnLead = 0.8 + Math.random() * 0.9;
      }

      // move + collide. Player box: x 42..54, y s.y-8 .. s.y+18.
      const pl = 42, pr = 54, pt = s.y - 8, pb = s.y + 18;
      s.bugs.forEach((b) => { b.x -= speed * dt; b.t += dt; });
      s.leads.forEach((l) => { l.x -= speed * dt; l.t += dt; });
      s.bugs = s.bugs.filter((b) => {
        if (b.x < -14) return false;
        if (s.inv <= 0 && b.x < pr && b.x + 10 > pl && pb > FQ.GROUND + 8) {
          s.hearts -= 1; s.inv = 1.2;
          if (s.hearts <= 0) {
            const sc = score(s);
            setFinalScore(sc); setBest((p) => Math.max(p, sc)); setPhase('over');
          }
          return false;
        }
        return true;
      ...
      s.leads = s.leads.filter((l) => {
        ...
        if (l.x < pr && l.x + 8 > pl && l.y < pb && l.y + 8 > pt) { s.coins += 1; return false; }
        return true;
      });

      // ship the site → level up
      const shippedNow = Math.floor(score(s) / FQ.SCORE_PER_SECTION);
      if (shippedNow > s.shipped) { s.shipped = shippedNow; s.lvl += 1; s.flash = 1.6; }
    };
    ...
    const onVis = () => { const s = g.current; if (s) s.last = 0; };
  ...
  return (
    <div
      ...
      onKeyDown={onKey}
      onPointerDown={() => { if (phaseRef.current === 'run') jump(); }}
    >
      <canvas ref={canvasRef} width={FQ.W} height={FQ.H} className="pv-game__canvas" />
      <span className="reel__scan" aria-hidden="true" />
      {phase === 'boot' && (
        <div className="pv-boot" role="status" aria-label="Opening Forge Quest">
          <div className="pv-boot__bar">
            <span>CLIENTFORGE OS</span>
            <span>09:41</span>
          </div>
          <div className="pv-boot__desktop">
            <div className="pv-boot__icon">
              <span className="pv-boot__folder" aria-hidden="true"><i /></span>
              <small>FORGE QUEST</small>
            </div>
            {bootStage === 'desktop' && (
              <span className="pv-boot__cursor" aria-hidden="true"><i /><b /></span>
            )}
            {bootStage === 'terminal' && (
              <div className="pv-terminal">
                <div className="pv-terminal__bar">
                  <span aria-hidden="true">■ ■ ■</span>
                  <span>TERMINAL</span>
                </div>
                <div className="pv-terminal__body">
                  <span className="pv-terminal__prompt">
                    clientforge@studio:~$ <span className="pv-terminal__command">forge-quest --launch</span>
                  </span>
                  <span className="pv-terminal__result pv-terminal__result--1">BUILD CHECK................OK</span>
                  <span className="pv-terminal__result pv-terminal__result--2">CONTROLS: SPACE / TAP TO JUMP</span>
                  <span className="pv-terminal__result pv-terminal__result--3">OPENING GAME...<i aria-hidden="true" /></span>
                </div>
              </div>
            )}
          </div>
      ...
      {phase === 'over' && (
        <div className="pv-game__overlay">
          <div className="pv-game__title">SITE CRASHED!</div>
          <div className="pv-game__sub">
            {'SCORE ' + finalScore + (best > finalScore ? ' · BEST ' + best : ' · NEW BEST!')}
          </div>
          <button type="button" className="pv-game__btn" onClick={start}>
            ↻ RETRY
          </button>
          <div className="pv-game__hint">SPACE / TAP TO JUMP</div>
        </div>
      )}
    </div>
  );
...
function PreviewCard({ slot, i }) {
  ...
  return (
    ...
    <div className="pv-card" style={{ '--d': (i * 90) + 'ms' }}>
      {slot.game ? (
        <div className="pv-frame pv-frame--game">
          <div className="pv-laptop">
            <div className="pv-laptop__lid">
              <img src="assets/game/forge-laptop-lid.png" className="pv-laptop__art" alt="" draggable="false" />
              <div className="pv-laptop__display">
                <ForgeGame />
              </div>
            </div>
            <img src="assets/game/forge-laptop-base.png" className="pv-laptop__base" alt="" draggable="false" />
          </div>
        </div>
      ) : (
        <button
        ...
        >
          ...
          {!broken && (
            <span className={'pv-play' + (playing ? ' is-playing' : '')}>
              {playing ? '❚❚' : 'PLAY'}
            </span>
          )}
          <span className="reel__scan" aria-hidden="true" />
      ...
      <div className="pv-foot">
        <span className="pv-foot__stat">
          <span className="pv-foot__icon" aria-hidden="true">{slot.icon}</span>
          {slot.stat}
        </span>
        <span className="pv-foot__title">{slot.title}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <section id="work" data-screen-label="03 Work" className="relative bg-black py-24 md:py-32">
      ...
      <div className="relative max-w-6xl mx-auto px-5 sm:px-8">
        ...
        <div className="flex items-center gap-3 text-[10px] md:text-xs uppercase tracking-[0.3em] text-primary/55 mb-8">
          <span className="inline-block w-6 h-px bg-primary/30" />
          <span>03 · Our Work</span>
        </div>

        {/* The arcade marquee: pixel title + small spec line, per the cabinet. */}
        <div className="pv-marquee">
          <span className="pv-marquee__flag" aria-hidden="true">▶</span>
          {/* Plain text, not WordsPullUpMultiStyle: the per-word pull-up leaves
              letters stranded mid-transform when the section is scrolled past
              quickly, and the pixel face reads as broken glyphs when it does.
              A marquee should be legible the instant it's on screen. */}
          <h2 className="pv-marquee__title">Select a Preview</h2>
          <FadeUp delay={0.2}>
            <span className="pv-marquee__spec">Website demos and walkthroughs</span>
          </FadeUp>
        </div>

        {/* Track tabs — two cartridges. */}
        ...
        <div className="pv-strip">
          <span className="pv-strip__gem" aria-hidden="true">◆</span>
          {active.blurb}
        </div>

        {/* 2×2 on desktop, single column on phones. Keyed on the track so
            switching remounts the cards and replays their entrance. */}
        <div className="pv-grid" key={active.id}>
          {active.slots.map((s, i) => (
            <PreviewCard key={s.id} slot={s} i={i} />
          ))}
        </div>

        <div className="pv-cta">
          <span className="pv-cta__copy">Want a build like these for your business?</span>
          <button type="button" className="reel__ask-btn pv-cta__btn" onClick={() => setAsking(true)}>
            Book a 15-min call
          </button>
        </div>
